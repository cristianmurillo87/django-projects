...
class FollowAPIView(APIView):
    def post(self, request, user_id, format=None):
        try:
            ...
            raise NotFound("You cannot follow a profile that does not exist.")
...
class UnfollowAPIView(APIView):
    def post(self, request, user_id, *args, **kwargs):
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                "status_code": status.HTTP_400_BAD_REQUEST,
                "message": f"You cannot unfollow {user_profile.user.first_name} {user_profile.user.last_name}, since you were not their follower.",
            }
            return Response(formatted_response, status=status.HTTP_400_BAD_REQUEST)
        ...
        formatted_response = {
            "status_code": status.HTTP_200_OK,
            "message": f"You are unfollowed {profile.user.first_name} {profile.user.last_name}",
        }
        return Response(formatted_response, status=status.HTTP_200_OK)
